...
def read_snapshot(snapshot_id: int):
    ...
    # ---------------------------------------------------------
    # Decode stored payloads
    # ---------------------------------------------------------

    raw_details = decode(row["details_json"]) or []
    raw_exclusions = decode(row["exclusions_json"]) or []
    ...
    # Normalize details
    #
    # Snapshot files are historical. Newer versions of the
    # application may expect fields that older snapshots did not
    # store. Keep all compatibility logic here.
    # ---------------------------------------------------------
    ...
    for detail in raw_details:
        ...
        # Pressure / order-book fields
        # -----------------------------------------------------
        # Backward compatibility: pressure / order book
        # -----------------------------------------------------

        # Old snapshots called pressure "imbalance".
        if "pressure_z" not in item:
            item["pressure_z"] = item.get("imbalance")

        item.setdefault("bid_depth", 0.0)
        item.setdefault("ask_depth", 0.0)

        # Old snapshots did not store depth_scale.
        item.setdefault("depth_scale", None)

        # -----------------------------------------------------
        # Backward compatibility: prices
        # -----------------------------------------------------

        if "model_price" not in item:
            item["model_price"] = item.get("estimated_price")

        if "anchor_price" not in item:
            item["anchor_price"] = item.get("reference_price")

        # -----------------------------------------------------
        # Backward compatibility: returns
        # -----------------------------------------------------

        if "model_return_pct" not in item:
            item["model_return_pct"] = item.get("return_pct")

        if "anchor_return_pct" not in item:
            item["anchor_return_pct"] = item.get("last_return_pct")

        # Order-book state
        item.setdefault("book_valid", False)
        item.setdefault("carried", False)

        # Price-method metadata
        item.setdefault("price_method", None)

        # Price fields
        item.setdefault("anchor_price", None)
        item.setdefault("model_price", None)
        item.setdefault("last_trade", None)

        # Base prices
        item.setdefault("anchor_base", None)
        item.setdefault("model_base", None)

        # Returns
        item.setdefault("anchor_return_pct", None)
        item.setdefault("model_return_pct", None)
        ...
        item.setdefault("asset_code", "")
        item.setdefault("scale_source", "fallback")

        # Scope weights
        for scope_name in SCOPES:
            item.setdefault(f"weight_{scope_name}", 0.0)
    ...
    for v in values:
        ...
        # Values stored in the current SQLite columns always win.
        item.update({
            "scope": v["scope"],
            "points": v["points"],
        ...
        if "base_at" not in item:
            item["base_at"] = row["fetched_at"]

        if "previous_data_day" not in item:
            item["previous_data_day"] = None

        if "price_points" not in item:
            return_pct = item.get("last_return_pct")

            if return_pct is not None:
                item["price_points"] = (
                    100 * (1 + float(return_pct) / 100)
                )
            else:
                item["price_points"] = None

        if "carried_weight_pct" not in item:
            item["carried_weight_pct"] = 0.0

        if "fallback_scale_weight_pct" not in item:
            item["fallback_scale_weight_pct"] = 0.0
